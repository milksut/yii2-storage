...
(function () {
  ...
  window.createShare = function (type) {
    ...
    if (type === "user") {
      ...
      data.permission_level = document.getElementById(
        "shareUserPermission",
      ).value;
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          if (type === "workspace")
            document.getElementById("shareWorkspaceSelect").value = "";
        } else {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          if (
            document.querySelectorAll("#sharesList [data-share-id]").length ===
            0
          ) {
            const noSharesMsg = config.messages.noSharesYet || "No shares yet";
            document.getElementById("sharesList").innerHTML =
              '<div class="text-muted text-center py-3"><i class="fa fa-info-circle me-1"></i> ' +
              noSharesMsg +
              "</div>";
          }
        } else {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          const statusText =
            level === "public"
              ? config.messages.public
              : config.messages.restricted;
          const msg = config.messages.accessChanged.replace(
            "{status}",
            statusText,
          );
          showShareToast(msg);
  ...
  window.handleCopyLink = function (btn) {
    ...
    function onCopied() {
      ...
      btn.innerHTML =
        '<i class="fa fa-check me-2"></i>' + (btn.dataset.copied || "Copied!");
